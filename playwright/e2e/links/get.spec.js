...
test.describe('GET /links/', () => {
    ...
    test('should return a list of links', async ({ authorization, links }) => {
        ...
        for (const [index, link] of body.data.entries()) {
            ...
            // expect(link.orriginal_url).toBe(user.links[index].original_url);
            expect(link).toHaveProperty('short_code');
            // expect(link.title).toBe(user.links[index].title);
    ...
    test('should not return links from another user', async ({ authorization, links }) => {
    ...
    test('should isolate links between different users', async ({ authorization, links }) => {
        const user1 = getUserWithLinks(3);
        const user2 = getUserWithLinks(2);
...
// Error: Falha ao criar link: {"message":"O campo 'OriginalURL' é obrigatório"}
    test('should return 401 when using expired token', async ({ authorization, links }) => {
        ...
        // const response = await links.getLinks(token2);
        console.log(user1.id, token1, user2, token2);

        const response = await links.getLinkId(user1.id, token2);
